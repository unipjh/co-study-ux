...
**Huge Problem(Pain Point):** 기존 학습 AI가 정보를 일방향으로 제공하면 학습자가 자기 언어로 설명하고 지식 틀을 만드는 과정이 약해진다.

**솔루션 한 줄 정의:** 사용자가 먼저 설명하고 AI가 반응하는 학습 경험을 디자인씽킹 자료로 검증·축적한다.

**현재 상태:** UX 리서치 아카이브

**문제 해결 중심의 사고 흐름**

1. **관찰** — 같은 AI를 사용해도 학습자가 가진 개념 틀과 설명 행동에 따라 학습 결과가 달라졌다.
2. **선택** — AI의 답변 품질보다 사용자 설명을 먼저 끌어내는 상호작용을 핵심 가설로 정했다.
3. **구현** — 경험 인터뷰, 경쟁 UX 분석, VPC, 프로토타입과 테스트 자료를 React·Firebase 아카이브에 연결했다.
4. **검증과 한계** — Empathize부터 Test까지 산출물을 추적할 수 있다. 표본 규모와 정량 효과는 제한점으로 남긴다.
